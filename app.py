...
def get_top3_feature_importance(rf_model):
    ...
    CATATAN: Ini adalah GLOBAL importance (sama untuk semua mahasiswa).
    Untuk penjelasan INDIVIDUAL, gunakan get_shap_contributions().
    """
...
def _extract_class1_shap(shap_raw, n_features):
    """
    Menyeragamkan output shap_values menjadi array 2D (n_samples, n_features)
    untuk kelas 1 (Tidak Melanjutkan), terlepas dari format output yang
    dikembalikan oleh versi SHAP yang berbeda:
      - SHAP versi lama  -> list [array(n,f), array(n,f)] per kelas
      - SHAP versi baru  -> array 3D (n, f, n_kelas)
    """
    if isinstance(shap_raw, list):
        # Format lama: list per kelas
        return np.asarray(shap_raw[1])
    arr = np.asarray(shap_raw)
    if arr.ndim == 3:
        # Format baru: (n_samples, n_features, n_kelas) -> ambil kelas index 1
        return arr[:, :, 1]
    # Fallback: sudah 2D (kasus model biner yang hanya mengembalikan 1 set)
    return arr


def get_shap_contributions(x_input: pd.DataFrame, top_n: int = 3):
    """
    Menghitung kontribusi SHAP individual untuk SATU baris data mahasiswa.
    Mengembalikan DataFrame berisi top_n fitur dengan kontribusi terbesar
    terhadap probabilitas kelas 1 (Tidak Melanjutkan / Berisiko), beserta
    arah pengaruhnya (meningkatkan atau menurunkan risiko).

    INILAH PENGGANTI model.feature_importances_ — hasil yang dikembalikan
    BERBEDA untuk setiap mahasiswa karena dihitung dari data spesifiknya.
    """
    shap_raw = explainer.shap_values(x_input)
    sv_class1 = _extract_class1_shap(shap_raw, len(FITUR_MODEL))[0]  # baris ke-0

    kontribusi = pd.DataFrame({
        "Fitur": FITUR_MODEL,
        "SHAP_Value": sv_class1,
    })
    kontribusi["Label"] = kontribusi["Fitur"].map(LABEL_FITUR)
    kontribusi["Abs_Value"] = kontribusi["SHAP_Value"].abs()
    total_abs = kontribusi["Abs_Value"].sum()
    kontribusi["Persentase"] = (
        kontribusi["Abs_Value"] / total_abs * 100 if total_abs > 0 else 0
    )
    kontribusi["Arah"] = np.where(
        kontribusi["SHAP_Value"] > 0, "meningkatkan risiko", "menurunkan risiko"
    )
    kontribusi = kontribusi.sort_values("Abs_Value", ascending=False).reset_index(drop=True)
    return kontribusi.head(top_n)


def get_shap_contributions_batch(x_batch: pd.DataFrame, top_n: int = 3):
    """
    Menghitung kontribusi SHAP untuk SELURUH baris pada data CSV sekaligus
    (vektorisasi, jauh lebih efisien daripada memanggil SHAP per baris).
    Mengembalikan list string siap pakai, satu per baris, contoh:
      "IPK_Saat_Ini (27.3%), Status_Keuangan (21.1%), Bimbingan_Pending (14.5%)"
    """
    shap_raw = explainer.shap_values(x_batch)
    sv_class1 = _extract_class1_shap(shap_raw, len(FITUR_MODEL))  # (n_baris, n_fitur)

    hasil_teks = []
    for row in sv_class1:
        s = pd.Series(row, index=FITUR_MODEL)
        abs_s = s.abs()
        total = abs_s.sum()
        pct = (abs_s / total * 100) if total > 0 else abs_s
        top = pct.sort_values(ascending=False).head(top_n)
        bagian = [f"{LABEL_FITUR[f]} ({v:.1f}%)" for f, v in top.items()]
        hasil_teks.append(", ".join(bagian))
    return hasil_teks
...
with tab_manual:
    ...
    if klik_prediksi:
        ...
        if not is_valid:
            ...
            # ---------- Top 3 Faktor Paling Berpengaruh (SHAP — Local Explanation) ----------
            st.markdown("#### Faktor Paling Berpengaruh untuk Mahasiswa Ini")
            st.caption(
                "Dihitung menggunakan SHAP (SHapley Additive Explanations) — "
                "hasil bersifat individual dan akan berbeda untuk setiap mahasiswa, "
                "karena dihitung dari kombinasi data spesifiknya, bukan dari "
                "rata-rata seluruh dataset."
            )
            top3_shap = get_shap_contributions(x_input, top_n=3)
            for rank, row in enumerate(top3_shap.itertuples(), start=1):
                ikon = "🔺" if row.Arah == "meningkatkan risiko" else "🔻"
                st.markdown(
                    f"**{rank}. {row.Label}** — {row.Persentase:.1f}% "
                    f"({ikon} {row.Arah})"
                )
            ...
            if df_upload is not None:
                ...
                if proses_csv:
                    ...
                    if not is_valid:
                        ...
                        # SHAP — faktor individual per mahasiswa (vektorisasi,
                        # dihitung sekali untuk seluruh batch, bukan per baris)
                        ...
                        hasil_df["Top_3_Faktor_Individual"] = faktor_individual
